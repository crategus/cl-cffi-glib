...
;;; ----------------------------------------------------------------------------
;;; enum GUserDirectory                                    not exported
;;; ----------------------------------------------------------------------------

(defcenum user-directory
  :desktop
  :documents
  :download
  :music
...
#+liber-documentation
(setf (liber:alias-for-type 'user-directory)
      "CEnum"
      (documentation 'user-directory 'type)
 "@version{2022-11-23}
  @begin{short}
    These are logical IDs for special directories which are defined depending
    on the platform used.
  @end{short}
  You should use the @fun{g:user-special-dir} function to retrieve the full
...
;;; ----------------------------------------------------------------------------
;;; g_get_environ ()                                       not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_environ" environ) (strv-t :free-from-foreign t)
 #+liber-documentation
 "@version{2022-11-23}
  ...
  @end{short}
  Each item in the list is of the form '@code{NAME} = @code{VALUE}'. This is
  equivalent to direct access to the 'environ' global variable, except portable.
...
;;; ----------------------------------------------------------------------------
;;; g_getenv ()                                            not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_getenv" getenv) :string
 #+liber-documentation
 "@version{2022-11-23}
  ...
  @return{A string with the value of the environment variable, or @code{nil} if
    ...
    overwritten by the next call to the @sym{g:getenv} or @fun{g:setenv}
    functions.}
...
;;; ----------------------------------------------------------------------------
;;; g_setenv ()                                            not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_setenv" setenv) :boolean
 #+liber-documentation
 "@version{2022-11-23}
  ...
  @begin[Warning]{dictionary}
    ...
    may crash if one thread calls the @sym{setenv} function while another
...
;;; ----------------------------------------------------------------------------
;;; g_listenv ()                                           not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_listenv" listenv) strv-t
 #+liber-documentation
 "@version{2022-11-23}
  ...
  function and the @fun{g:getenv} function instead of using the environ array
  ...
  this function and the @fun{g:getenv} function provide.
  @see-function{g:getenv}")

;;; ----------------------------------------------------------------------------
;;; g_get_user_name () -> user-name                        not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_user_name" user-name) :string
 #+liber-documentation
 "@version{2022-11-23}
...
;;; ----------------------------------------------------------------------------
;;; g_get_real_name () -> real-name                        not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_real_name" real-name) :string
 #+liber-documentation
 "@version{2022-11-23}
...
;;; ----------------------------------------------------------------------------
;;; g_get_user_cache_dir () -> user-cache-dir              not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_user_cache_dir" user-cache-dir) :string
 #+liber-documentation
 "@version{2022-11-23}
...
;;; ----------------------------------------------------------------------------
;;; g_get_user_data_dir ()                                 not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_user_data_dir" user-data-dir) :string
 #+liber-documentation
 "@version{2022-11-23}
  @return{A string owned by GLib that must not be modified or freed.}
  ...
  on Windows it thus is the same as what the @fun{g:user-config-dir} function
  returns.
...
;;; ----------------------------------------------------------------------------
;;; g_get_user_config_dir () -> user-config-dir            not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_user_config_dir" user-config-dir) :string
 #+liber-documentation
 "@version{2022-11-23}
  @return{A string owned by GLib that must not be modified or freed.}
  ...
  On Windows this is the folder to use for local (as opposed to roaming)
  application data. See documentation for @code{CSIDL_LOCAL_APPDATA}. Note that
  on Windows it thus is the same as what the @fun{g:user-data-dir} function
  returns.
  @see-function{g:user-cache-dir}
  @see-function{g:user-data-dir}
  @see-function{g:user-runtime-dir}
...
;;; ----------------------------------------------------------------------------
;;; g_get_user_runtime_dir () -> user-runtime-dir          not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_user_runtime_dir" user-runtime-dir) :string
 #+liber-documentation
 "@version{2022-11-23}
  @return{A string owned by GLib that must not be modified or freed}
  @begin{short}
  ...
  of the @fun{g:user-cache-dir} function.
...
;;; ----------------------------------------------------------------------------
;;; g_get_user_special_dir () -> user-special-dir          not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_user_special_dir" user-special-dir) :string
 #+liber-documentation
 "@version{2022-11-23}
...
;;; ----------------------------------------------------------------------------
;;; g_get_system_data_dirs () -> system-data-dirs          not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_system_data_dirs" system-data-dirs)
    (strv-t :free-from-foreign nil)
 #+liber-documentation
 "@return{A list of strings with base directoires.}
...
;;; ----------------------------------------------------------------------------
;;; g_get_system_config_dirs () -> system-config-dirs      not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_system_config_dirs" system-config-dirs)
    (strv-t :free-from-foreign nil)
 #+liber-documentation
 "@version{2022-11-23}
  @return{A list of strings with base directories.}
  @short{Returns an ordered list of base directories in which to access
...
;;; ----------------------------------------------------------------------------
;;; g_get_host_name ()                                     not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_host_name" host-name) :string
 #+liber-documentation
 "@version{2022-11-23}
  ...
  running, the return value from this function does not change.If no name can
...
;;; ----------------------------------------------------------------------------
;;; g_get_home_dir () -> home-dir                          not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_home_dir" home-dir) :string
 #+liber-documentation
 "@version{2022-11-23}
...
;;; ----------------------------------------------------------------------------
;;; g_get_tmp_dir () -> tmp-dir                            not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_tmp_dir" tmp-dir) :string
 #+liber-documentation
 "@version{2022-11-23}
...
;;; ----------------------------------------------------------------------------
;;; g_get_current_dir () -> current-dir                    not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_get_current_dir" current-dir) (:string :free-from-foreign t)
 #+liber-documentation
 "@version{2022-11-23}
...
;;; ----------------------------------------------------------------------------
;;; g_path_is_absolute ()                                  not exported
;;; ----------------------------------------------------------------------------

(defcfun ("g_path_is_absolute" path-is-absolute) :boolean
 #+liber-documentation
 "@version{2022-11-24}
  @argument[filename]{a string with a file name}
  @return{@em{True} if @arg{filename} is absolute.}
  @begin{short}
    Returns @em{true} if the given @arg{filename} is an absolute file name.
  @end{short}
  ...
  by the @code{getcwd()} or @fun{g:current-dir} functions either. Such paths
...
(defcfun ("g_build_filenamev" %build-filenamev) (:string :free-from-foreign t)
  (args strv-t))
...
;;; ----------------------------------------------------------------------------
;;; g_build_path ()                                        not exported
;;; ----------------------------------------------------------------------------

(defun build-path (separator &rest args)
 #+liber-documentation
 "@version{2022-11-23}
...
(defcfun ("g_build_pathv" %build-pathv) (:string :free-from-foreign t)
  (separator :string)
  (args strv-t))
