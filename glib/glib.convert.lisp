...
;;; glib.convert.lisp
;;;
;;; The documentation of this file is taken from the GLib 2.72 Reference Manual
;;; and modified to document the Lisp binding to the GLib library.
...
(defcfun ("g_filename_to_utf8" %filename-to-utf8) :string
  (str :string)
  (len :int)
  (bytesread (:pointer :size))
  (byteswritten (:pointer :size))
  (err :pointer))

(defun filename-to-utf8 (str)
 #+liber-documentation
 "@version{#2022-11-3}
  @argument[str]{a string in the encoding for filenames}
  ...
  (with-g-error (err)
    (with-foreign-objects ((bytesread :size) (byteswritten :size))
      (%filename-to-utf8 str -1 bytesread byteswritten err))))

(export 'filename-to-utf8)

;;; ----------------------------------------------------------------------------
;;; g_filename_from_utf8 ()
;;; ----------------------------------------------------------------------------

(defcfun ("g_filename_from_utf8" %filename-from-utf8) :string
  (uft8 :string)
  (len :int)
  (bytesread (:pointer :size))
  (byteswritten (:pointer :size))
...
(defun filename-from-utf8 (utf8)
 #+liber-documentation
 "@version{#2022-11-3}
  @argument[utf8]{a UTF-8 encoded string}
  @return{The converted string, or @code{nil} on an error.}
  @begin{short}
  ...
  (with-g-error (err)
    (with-foreign-objects ((bytesread :size) (byteswritten :size))
      (%filename-from-utf8 utf8 -1 bytesread byteswritten err))))

(export 'filename-from-utf8)
